...
from tools.multi_head_attention import MultiHeadAttention, MultiHeadAttentionChunk, MultiHeadAttentionWindow
from tools.positionwise_feed_forward import PositionwiseFeedForward
...
class Encoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
        ...
        mask:
            Attention mask tensor. Can be:
            - None: No masking
            - 'subsequent': Causal mask for autoregressive models
            - torch.Tensor: Custom padding mask with shape (batch_size, K)
        ...
        """
        # Self attention - the mask is now propagated
        residual = x
        x = self._selfAttention(query=x, key=x, value=x, mask=mask)
        x = self._dopout(x)
