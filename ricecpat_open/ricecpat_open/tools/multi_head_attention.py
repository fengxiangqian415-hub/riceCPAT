...
from tools.positional_encoding import generate_local_map_mask
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        mask:
            ...
            Can be:
            - 'subsequent': Causal mask for autoregressive models  
            - torch.Tensor: Padding mask with shape (batch_size, K) where 1=real token, 0=padding
            - None: No masking
        ...
        # Handle a padding mask
        if mask is not None and isinstance(mask, torch.Tensor):
            # Assume mask is a padding mask: (batch_size, seq_len)
            # Expand it to (batch_size * h, seq_len, seq_len)
            if mask.dim() == 2:  # (batch_size, seq_len)
                batch_size = mask.size(0)
                seq_len = mask.size(1)
                
                # Build the attention mask: (batch_size, seq_len, seq_len)
                # If a key position is padding (mask=0), its attention score is set to -inf
                key_mask = mask.unsqueeze(1)  # (batch_size, 1, seq_len)
                key_mask = key_mask.expand(-1, seq_len, -1)  # (batch_size, seq_len, seq_len)
                
                # Repeat for all heads
                key_mask = key_mask.unsqueeze(1).repeat(1, self._h, 1, 1)  # (batch_size, h, seq_len, seq_len)
                key_mask = key_mask.view(batch_size * self._h, seq_len, seq_len)
                
                # Apply the mask: padding positions are set to -inf
                self._scores = self._scores.masked_fill(key_mask == 0, float('-inf'))

        # Apply sotfmax
        self._attention_weights = F.softmax(self._scores, dim=-1)

        attention = torch.bmm(self._attention_weights, values)
